...
vm.runInContext('globalThis.__L = lighting; globalThis.__W = world;', context); const L = context.__L; let sum=0; for (let i=0;i<L.cols*L.rows;i++) sum+=L.grid[i]; console.log('grid mean', (sum/(L.cols*L.rows)).toFixed(3), 'origin', L.originX, L.originY, 'sky', L.skyHeight[440], 'cam', Math.round(g.camera.x/32), Math.round(g.camera.y/32));
